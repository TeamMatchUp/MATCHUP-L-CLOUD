...
export function GymsNearYouWidget({ fighterProfileId }: GymsNearYouWidgetProps) {
    ...
    return (
      <div className="flex flex-col flex-1">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          <LocateFixed style={{ width: 32, height: 32, color: "hsl(var(--muted-foreground))", marginBottom: 8 }} />
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col flex-1">
        {heading}
        <div className="space-y-2">{[1, 2, 3].map((i) => (<div key={i} style={{ height: 56, background: "hsl(var(--muted))", borderRadius: 8 }} className="animate-pulse" />))}</div>
      </div>
    );
  }
  ...
  if (nearbyGyms.length === 0) {
    return (
      <div className="flex flex-col flex-1">
        {heading}
        <p style={{ fontSize: 12, color: "hsl(var(--muted-foreground))", textAlign: "center", padding: "16px 0" }}>No gyms found within 20 miles.</p>
      </div>
    );
  }
